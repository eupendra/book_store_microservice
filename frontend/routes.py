...
#        session.pop('user', None)  # todo check why session is not cleared on app terminate
    try:
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            if api_key:
                ...
                flash(f"Welcome back, {session['user']['username']}!")
                return redirect(url_for('frontend.index'))
            else:
                flash('Cannot Login')
        else:
...
def cart():
    ...
    order_items = order['result']['order_items']
    cart_items=[]
    total_price=0
    for order_item in order_items:
        cart_item=order_item
        cart_item['book']=BookClient.get_book_by_id(order_item['book']).get('result')
        total_price += (cart_item['book'].get('price') * cart_item['quantity'])
